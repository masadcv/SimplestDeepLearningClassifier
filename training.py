...
def main():
    ...
    for vm in train_config["val_metrics"]:
        metrics_dict[vm] = core.metrics.get_metric(
            vm, num_classes=dataloader_config["num_classes"], average="macro"
        ).to(device)
    args["val_metrics"] = metrics_dict
    args["conf_matrix_func"] = core.metrics.get_metric(
        "CONF", num_classes=dataloader_config["num_classes"]
    )
    best_loss_val = 1e10
